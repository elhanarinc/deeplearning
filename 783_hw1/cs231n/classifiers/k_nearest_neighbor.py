...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    # (x-y)^2 means that x^2 + y^2 - 2xy
    # This is 2xy
    Two_X_and_X_train = X.dot(self.X_train.T)

    # This is x^2, also we need to preserve its dimensions because we need 500 different rows
    X_square = np.sum(np.square(X), axis = 1, keepdims = True)

    # This is y^2, we need to convert this as a 5000 element array
    X_train_square = np.sum(np.square(self.X_train), axis = 1)

    # With the addition, we will apply 5000 elements to each row in 500 element array
    dists = np.sqrt(X_square + X_train_square - 2 * Two_X_and_X_train)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      for indice in neighbor_indices:
